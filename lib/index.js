...
function recenterModal(modal) {
    try {
        ...
        var x = Math.floor(mainBounds.x + ((mainBounds.width  - modalBounds.width ) / 2));
        var y = Math.floor(mainBounds.y + ((mainBounds.height - modalBounds.height) / 2));
        console.log('recenterModal()', 'x=', x, 'y=', y);
        modal.setPosition(x, y);
    } catch (error) {
        console.log('recenterModal()', error.message);
    }
}
...
ipcMain.on('post-json', (event, payload) => {
    ...
    request.on('response', (response) => {
        console.log(payload.endpoint, `Status: ${response.statusCode}`);
        response.on('error', (error) => {
            console.log(payload.endpoint, `Error: ${JSON.stringify(error)}`);
        })
    });
