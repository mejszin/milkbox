...
function addTrack(index, data) {
    ...
    for (let i = 0; i < data.length + 1; i++) {
        ...
        if (i < data.length) {
            ...
            // Buttons
            cell.innerText = 'Add to playlist'
            cell.onclick = function() { addTrackToPlaylist(now_playing_tracks[index]) }
        }
        row.appendChild(cell);
    }
    table.appendChild(row);
...
async function displayTracksPage() {
    ...
    loadAllTracks(music_root_dir).then((track_paths) => {
...
async function displayAlbumsPage() {
    ...
    clearTracklist();
    displayAlbumsHeader();
    loadAllAlbums(music_root_dir).then((albums) => {
