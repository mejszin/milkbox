...
function addTracklistHeader() {
    ...
    var cell = null;
    const data = [
        "Title", "Artist", "Album", "Year", "Genres", "Track", "Duration"
    ]
    const columns = [
        'title', 'artist', 'album', 'year', 'genres', 'track_no', 'duration',
    ]
    ...
    glob(`${dir}/**/*.*`, async function (error, files) {
        for (file_path of files) {
            try {
                ...
                genres = (genres == undefined) ? '' : genres.slice(0, 3).join(', '); 
                addTrack(
                    track_paths.length, [
                    ...
                    genres,             // Genres
                    `#${metadata.common.track.no}`,            // Track No
        ...
        td.innerText = `${track_paths.length} total tracks`;
    })
}
